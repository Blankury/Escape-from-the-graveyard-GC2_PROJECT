...
public:
	...
	void DrawText(float posX, float posY, string texto, float adjustment) {
		...
		for (int i = 0; i < texto.length(); i++) {
			...
			int ascii = int(texto.at(i)); //obtenemos el velor del caracter en ascii 

			float uStep = 0.03125; // definir el ancho de las letras en el mapa
			float vStep = 0.33334; // definir el alto de la plantilla
			float uLeft, URight, vTop, VBottom;

			// se define en que cual renglon/linea se encuentra el simbolo/letra

			if (ascii <= 63) {
				// estamos en la primera fila (todo esto dado a la tabla de ascii,32 elementos x cada linea)
				vTop = 0; // el top es 0 pq lit esta al inicio dd
				VBottom = vStep;
				...
				//tercera fila 
				vTop = 2 * vStep;
				VBottom = 3 * vStep; // o vBottom=1, porque definimos que el origen era 0 

				ascii -= 96;
			}

			uLeft = ascii * uStep;
			URight = (ascii * uStep) + uStep; // o uRigh = uLeft + uStep
