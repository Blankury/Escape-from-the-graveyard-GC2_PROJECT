...
{  
    ...
    vsOut.tex0 = float2(vertex.tex0.x, vertex.tex0.y) * 5.0;
...
float4 PS_Main(PS_Input pix) : SV_TARGET
{
	...
	////Usamos la variable movimiento que moverá la distorsión en x y y para obtener valores aleatorios
    float2 distortion = (dispMap.Sample(colorSampler, float2(pix.tex0.x + pix.movimiento, pix.tex0.y)).rg * 2.0 - 1.0) * ola;
    float2 distortion2 = (dispMap.Sample(colorSampler, float2(-pix.tex0.x + pix.movimiento, pix.tex0.y + pix.movimiento)).rg * 2.0 - 1.0) * ola;
    ...
    //para que la textura no se vea recortada 
    ...
    float4 AportLuzDif = saturate(LuzDifusa * FALL);
