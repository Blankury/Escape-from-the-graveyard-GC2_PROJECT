...
public:
	LagoRR(WCHAR* billb, WCHAR* normal, WCHAR* disp, ID3D11Device* D3DDevice, ID3D11DeviceContext* D3DContext, float escala)
	{
		...
		CargaParametros(billb, normal, disp, escal);
	}
	...
	bool CargaParametros(WCHAR* billb, WCHAR* normal, WCHAR* disp, float escala)
	{
		HRESULT d3dResult;
		ID3DBlob* vsBuffer = 0;
		...
		bool compileResult = CompileD3DShader(L"Lago.hlsl", "VS_Main", "vs_4_0", &vsBuffer);
		...
		compileResult = CompileD3DShader(L"Lago.hlsl", "PS_Main", "ps_4_0", &psBuffer);
		...
		//de movimiento
		d3dResult = d3dDevice->CreateBuffer(&constDesc, 0, &movCB);
		...
		float escala, vector2* uv1, vector2* uv2, vector2* uv3, vector2* uv4, float rot, char angler)
	{
		...
		d3dContext->UpdateSubresource(movCB, 0, 0, &movimiento, 0, 0);
		d3dContext->UpdateSubresource(rotCB, 0, 0, &rotationMat, 0, 0);
